...
def crackClients(clients, usersMac, SSID, passphraseQ):
    ...
    for clientHandshake in clientHandshakes:
        if clientHandshake[0]['AP'] == usersMac:
            cracked = crack(SSID, clientHandshake[0]['client'], clientHandshake[0]['AP'], clientHandshake[0]['Anonce'], clientHandshake[1]['Snonce'], clientHandshake[1]['mic'], clientHandshake[1]['data'])
            if cracked != False:
                return cracked
    return False
...
if __name__ == "__main__":
    ...
    for opt, arg in opts:
        if opt == '-r':
            readFile = arg
        if opt == '-m':
            usersMac = bytes.fromhex(arg.replace(":", ""))
        if opt == '-s':
            SSID = arg
        if opt == '-d':
            try:
